...
class InterData():
    ...
    def userLogin(self, data):
        ...
        if self.sett.terminal == "boli6.00":
            ...
            rtnData["dataString"] = "无效的客户端类型[{code}].".format(code=self.sett.clientCode)
    ...
    def basicDataGet(self, sType):
        ...
        if sType == 'dishCategory':
            ...
            ldKey["dishItem"] = ["cFood_C", "cFood_N", "cLitCls_C", "sNameFast", "sUnit", "bSuitFood", "eSuitType", "bMultiUnit", "bTimePrc", "nPrc", "nPrcRoom", "nPrcRoom2", "nPrcRoom3", "nPrcRoom4", "nPrcVip1", "nPrcVip2", "nPrcVip3", "nFee", "nPrcRoom4"]
        ...
        if len(rtnData["dataString"]) == 0:
            try:
                ...
                cur = conn.cursor()
                if not cur:
                    rtnData["dataString"] = "基础数据获取失败：{name}数据库[{db}]连接失败".format(name=self.sett.serverName, db=self.sett.serverDb)
                else:
