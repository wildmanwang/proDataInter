...
class SuperBillDCB():
    ...
    def _readRtnData(self, station, operType, data, judgeType = 0, judgeStr = "", judgeLine = 1):
        ...
        :param judgeStr: 当type=1时有用，返回结果匹配到指定字符串则执行成功，否则，返回结果第一个字符=1则执行成功
        :param judgeLine: 判断行数 当type=1时有用，以返回信息第几行为标准判断是否成功
        :return:
        """
        rtnData = {
            "result":True,                 # 逻辑控制 True/False
                ...
                if readCon[0][:8] == data[0][:8]:
                    if judgeType == 0:
                        if readCon[judgeLine][0] != "1":
                            rtnData["result"] = False
                    elif judgeType == 1 and len(judgeStr) > 0 and judgeLine >= 0:
