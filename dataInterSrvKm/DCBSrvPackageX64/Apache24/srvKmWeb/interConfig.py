...
class Settings():
    ...
    def __init__(self):
        ...
        # 数据库服务器
        self.serverCode = config.getint("dbServer", "serverCode")    # 服务端代码，支持类型：
                                                                            # 0：标准点菜宝接口
                                                                            # 1：科脉好餐谋
        if self.serverCode == 0:
            self.serverName = "标准点菜宝"
        elif self.serverCode == 1:
            self.serverName = "科脉好餐谋"
        else:
            self.serverName = "未知数据源"
