...
class MyWindow(tkinter.Frame):
    def __init__(self, master=None):
        ...
        # 用于窗口创建
        self.master.lpfnWndProc = self.winProc
        self.title = "平板点餐服务YD"
        self.icon = "data.ico"
        self.pack()
        self.windows_init()
    ...
    def windows_init(self):
        ...
        self.master.geometry("360x250+200+120")
        self.numProcess = []
        self.txtLabel = []
        self.numLabel = []
        for i in range(8):
            newLabel = tkinter.Label(self.master, text="数据"+str(i+1)+":", font=("仿宋", 12))
            self.txtLabel.append(newLabel)
            self.numProcess.append(0)
            newLabel = tkinter.Label(self.master, text=str(self.numProcess[i]), font=("仿宋", 12))
            self.numLabel.append(newLabel)
    ...
    def create_widgets(self):
        ...
        i = 0
        for txtLabel in self.txtLabel:
            i += 1
            txtLabel.place(x=self.x0+self.colWidth*((i+1)%2), y=self.y0+self.lineHight*((i-1)//2))

        i = 0
        for numLabel in self.numLabel:
            i += 1
            numLabel.place(x=self.x0+self.colWidth*((i+1)%2)+self.labelWidth, y=self.y0+self.lineHight*((i-1)//2))

        self.hideButton = tkinter.Button(self.master, font=("仿宋", 12), text="隐藏", command=self.master.iconify)
        self.hideButton.place(x=self.x0, y=self.y0+self.lineHight*5, width = 300, height=40)
...
class SysTrayIcon(object):
    ...
    def pro_message(self, hwnd, msg, wparam, lparam):
        print(wparam, lparam)
...
class _Main:
    def main(self):
        ...
        # 开始托盘程序嵌入
        self.root = window
        icons = os.getcwd() + '\\' + self.frame.icon
        hover_text = self.frame.title
        menu_options = ()
